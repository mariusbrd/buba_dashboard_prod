...
# - Funktional 1:1 zum Original (download_data_fixed.py) bzgl. Fetch/Resolve/Align/Speichern
# - Zusätze: end_date=auto (aktueller Monat), monatlicher Stempel, enable_index-Schalter
...
# --- Warnungen wie im Original deaktivieren ---
import urllib3
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# --- Zeitzone / current month helper ---
...
# -------------------------------------------------------------------
def should_run_this_month(scenario_dir: str | Path, stamp_name: str = ".scenario_month.stamp") -> bool:
    """
...
def mark_ran_this_month(scenario_dir: str | Path, stamp_name: str = ".scenario_month.stamp") -> None:
...
def ensure_analysis_data_on_startup() -> Path:
    ...
    """
    # 1) Falls bereits vorhanden: nichts tun
    if DEFAULT_ANALYSIS_XLSX.exists():
        return DEFAULT_ANALYSIS_XLSX

    # 2) Minimal-Workbook erzeugen und speichern
    SCENARIO_DATA_DIR.mkdir(parents=True, exist_ok=True)

    # Beispiel: leeres Gerüst (passe die Spalten an dein Projekt an)
    df_hist = pd.DataFrame(columns=["Date", "Gesamt GVB"])  # etc.
    with pd.ExcelWriter(DEFAULT_ANALYSIS_XLSX, engine=get_excel_engine()) as xw:
        ...
        # ggf. weitere Sheets: exog, meta, config …

    return DEFAULT_ANALYSIS_XLSX

def save_scenario_overrides(df_overrides) -> Path:
    SCENARIO_DATA_DIR.mkdir(parents=True, exist_ok=True)
    with pd.ExcelWriter(DEFAULT_SCENARIO_XLSX, engine=get_excel_engine()) as xw:
        df_overrides.to_excel(xw, index=False)
