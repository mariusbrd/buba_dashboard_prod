...
class ScenarioAnalysis:
    ...
    def _log(self, msg: str):
        # 1) Bevorzugt in die Scenario-Table loggen
        try:
        ...
        # 2) Fallback: zentraler Dashboard-Logger (wenn vorhanden)
        try:
            import logging
            logging.getLogger("GVB_Dashboard").info(msg)
            return
        except Exception:
            pass

        # 3) Letzter Fallback: stdout
        try:
            print(msg)
        except Exception:
            pass
    ...
    def _log_exog_usage(self, top_k: int = 12, *, log_once: bool = True) -> None:
        ...
        def _log(msg: str):
            try:
                self._log(msg)  # eigene Logger-Methode der Klasse
            except Exception:
                try:
                    Log.scenario_table(msg)  # noqa: F821
                except Exception:
        ...
        if cfg.use_cached_model and not force_retrain and ModelArtifact.exists(model_path):
            try:
                ...
                if ok:
                    ...
                    skip = True
                    print(f"    ✓ Verwende gecachtes Modell ({artifact.metadata['timestamp']})")
                else:
                    print("    ⚠ Cache inkompatibel → neu trainieren:")
                    for s in issues:
                        print("      -", s)
            except Exception as e:
                print(f"    ⚠ Laden fehlgeschlagen → neu trainieren: {e}")
    ...
    def nowcast_t0(self,
        ...
        """
        print("=" * 80)
        print("NOWCAST PIPELINE (t0) – Szenario-Analyse")
        print("=" * 80)

        # 1) Daten + Modell
        print("[1/3] Daten vorbereiten …")
        df_feats = self.load_and_prepare()
        print(f"    → {len(df_feats)} Quartale, {df_feats.shape[1]} Features")

        print("[2/3] Modell laden/trainieren …")
    ...
    def _train_best_model_nowcast(df_feats: pd.DataFrame, cfg: Config):
        ...
        print(f"  Grid-Search mit {len(grid)} Kombinationen …")
        ...
        try:
            print(f"  → Gewählt: DT | CV_RMSE={best_rmse:.3f} | EXOG-Share={exog_share:.1%} | "
                f"PenaltyScore={score_pen:.3f} | params={best_params}")
        except Exception:
            pass
