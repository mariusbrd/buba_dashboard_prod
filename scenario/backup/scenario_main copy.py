...
def run_scenario_forecast_with_analyzer(n_clicks, target_ui, tbl_cols, tbl_rows, scenario_preset):
    ...
    try:
        ...
        if hasattr(sa, "_df_feats") and isinstance(sa._df_feats, pd.DataFrame) and "Q_end" in sa._df_feats.columns:
            ...
            Log.scenario_table("[ScenarioForecast] WARNUNG: Analyzer-Features ohne Q_end – überspringe Trim.")
...
def apply_scenario_to_table_on_click(columns, n_clicks, selected_scenario, current_rows):
    ...
    def _compute_model_baseline_q1_q4():
        ...
        if cfg.target_col not in df_det.columns:
            try:
                Log.scenario_table(f"[ScenarioApply] ERROR: Spalte '{cfg.target_col}' fehlt.")
            except Exception:
        ...
        if not mask_valid.any():
            try:
                Log.scenario_table("[ScenarioApply] WARN: Kein gültiges Target im Datensatz – Baseline bleibt unverändert")
            except Exception:
        ...
        if df_hist.empty:
            try:
                Log.scenario_table("[ScenarioApply] WARN: Getrimmte Historie ist leer.")
            except Exception:
        ...
        if not exog_cols:
            ...
            try:
                Log.scenario_table("[ScenarioApply] WARN: Keine passenden Exog-Spalten gefunden – Baseline leer.")
            except Exception:
...
def init_exog_override_table_quarterly(pathname):
    ...
    if base_excel is None:
        ...
        try:
            Log.scenario_table("Keine Szenario-Datei in ./scenario/data/ gefunden – Tabelle wird nicht initialisiert.")
        except Exception:
            pass
        raise dash.exceptions.PreventUpdate
