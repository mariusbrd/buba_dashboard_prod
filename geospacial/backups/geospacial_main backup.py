...
import dash_ag_grid as dag  # moderne Tabelle

import plotly.graph_objects as go

#from .geospacial_viz import build_map_from_df
from .geospacial_viz import (
    build_map_from_df,
    build_empty_map,   # <--- hinzufügen
    # ...
)
                ...
                [
                    dbc.Col(
                        dbc.Card(
                            ...
                            style={"minHeight": "720px"},  # 600px * 1.2
                        ),
                        width=8,
                        className="d-flex",
                    ),
                    ...
                    dbc.Col(
                        dbc.Card(
                            [
                                ...
                                dbc.Collapse(
                                    [
                                        # Einstellungsleiste inkl. Upload über bestehenden Button
                                        dbc.CardBody(
                                            [
                                                dbc.Row(
                                                    [
                                                        dbc.Col(
                                                            width=6,  # leere linke Spalte
                                                        ),
                                                        dbc.Col(
                                                            [
                                                                dbc.ButtonGroup(
                                                                    [
                                                                        ...
                                                                        # Bestehender "Daten hochladen"-Button als Upload
                                                                        dcc.Upload(
...
def register_geo_callbacks(app):
    ...
    @app.callback(
        Output("geo-forecast-store", "data"),
    ...
    def update_forecast_table(forecast_json, map_input):
        ...
        # 1) Deutschland gesamt (immer 100 % Deutschland-Prognose)
        germany_row = {
        ...
        # 2) Regionen, falls vorhanden: Anteil + anteilige Prognose
        if region_shares is not None and not region_shares.empty:
            for _, r in region_shares.iterrows():
                ...
                for i, label in enumerate(period_labels):
                    ...
                    val = base_val * share if pd.notna(base_val) else None
        ...
        column_defs = [
            ...
            {"field": "Anteil_am_BIP", "headerName": "Anteil an Var (%)"},
        ]
