...
def safe_write_csv(df: pd.DataFrame, path: Union[str, Path], label: str = "CSV") -> None:
    ...
    try:
        ...
        LOGGER.info(f"{label} geschrieben: {path}")
    except Exception as e:
        LOGGER.warning(f"{label} konnte nicht geschrieben werden: {e}")
...
def build_future_design(
    ...
    LOGGER.info(f"[Design] Zukunfts-Design (roh): {fut_designs.shape[0]} Quartale, {fut_designs.shape[1]} Features")
    ...
    if X_cols is not None and df_feats is not None:
        ...
        LOGGER.info(f"[Design] Zukunfts-Design (final): {fut_designs.shape[0]} Quartale, {fut_designs.shape[1]} Features")
...
def _section_1_prepare_data(cfg: Config) -> dict:
    ...
    LOGGER.info(_sym(f"[S1] → {len(df_feats)} Quartale, {df_feats.shape[1]} Features"))
        ...
        if getattr(cfg, "use_downloader_exog", False):
            ...
            if out_path and os.path.exists(out_path):
                ...
                if getattr(cfg, "debug_exog", False):
                    LOGGER.info(f"[S1|Exog] harvested from: {out_path}")
                    LOGGER.info(f"[S1|Exog] columns={resolved_exogs}")
            else:
                if getattr(cfg, "debug_exog", False):
                    LOGGER.info("[S1|Exog] Keine Downloader-Output-Datei gefunden.")
            if resolved_exogs:
                cfg.selected_exog = resolved_exogs
                if getattr(cfg, "debug_exog", False):
                    LOGGER.info(f"[S1|Exog] cfg.selected_exog gesetzt (n={len(cfg.selected_exog)})")
    except Exception as e:
...
def _section_3_forecast_and_export(ctx: dict) -> tuple[pd.DataFrame, dict]:
    ...
    if getattr(cfg, "debug_exog", False):
        LOGGER.info(f"[S3] Effektive Exog-Wunschliste: {list(getattr(cfg, 'selected_exog', []) or [])}")
    ...
    LOGGER.info(_sym(f"[S3] → {len(fut_designs)} Zukunftsquartale (final)"))
